...
def confirm_user_email(newuser):
    ...
    try:
        if current_app.config["MAIL"]["MAIL_USE_SSL"]:
            server = smtplib.SMTP_SSL(current_app.config["MAIL"]["MAIL_HOST"],int(current_app.config["MAIL"]["MAIL_PORT"]))
        else:
            server = smtplib.SMTP(current_app.config["MAIL"]["MAIL_HOST"],int(current_app.config["MAIL"]["MAIL_PORT"]))
