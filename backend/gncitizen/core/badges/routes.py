...
def get_rewards(id):
    ...
    scores_query = ObservationModel.query.filter(
        ObservationModel.id_role == id).group_by(ObservationModel.id_program).values(
            ObservationModel.id_program.label('id'), func.count(
                ObservationModel.id_program).label('nb_obs'))
                ...
                if program["id_program"] == reward['id_program']:
                    for badge in reward['badges']:
                        if program["nb_obs"] >= badge["min_obs"]:
                            ...
                            badge['id'] = reward['type'] + '_prog'+ str(reward['id_program'])+ '_'+ str(id)
                            awarded_badges.append(badge)
                            id= id + 1
