...
from .models import (
    AreaModel,
    SpeciesSiteModel,
    SpeciesSiteObservationModel,
    SpeciesStageModel,
    StagesStepModel,
    MediaOnSpeciesSiteObservationModel,
    MediaOnStagesStepsModel,
    AreasAccessModel,
...
admin.add_view(
    AreasView(AreaModel, db.session, "6 - Areas", category="Enquêtes")
)
...
def prepare_list(
    data, with_geom=True, maximum_count=0, fields=None, with_centroid=False
):
    features = []
    for element in data:
        formatted = format_entity(
            element, with_geom, fields=fields, with_centroid=with_centroid
        )
...
def generate_observation(id_species_site_observation):
    ...
    observation = (
        db.session.query(SpeciesSiteObservationModel, UserModel.username)
        .join(
            SpeciesSiteModel,
            SpeciesSiteObservationModel.id_species_site
            == SpeciesSiteModel.id_species_site,
            full=True,
        )
        .join(
            UserModel,
            SpeciesSiteObservationModel.id_role == UserModel.id_user,
            full=True,
        )
        .filter(
    ...
    feature = get_geojson_feature(
        observation.SpeciesSiteObservationModel.species_site.geom
    )
    feature["properties"][
        "cd_nom"
    ] = observation.SpeciesSiteObservationModel.species_site.cd_nom

    feature["properties"][
        "id_species_site"
    ] = observation.SpeciesSiteObservationModel.id_species_site
    feature["properties"][
        "id_stages_step"
    ] = observation.SpeciesSiteObservationModel.id_stages_step
    feature["properties"][
        "date"
    ] = observation.SpeciesSiteObservationModel.date
...
def get_user_areas():
    ...
    try:
        ...
        areas = (
            ...
            .outerjoin(
                SpeciesSiteModel, AreaModel.id_area == SpeciesSiteModel.id_area
            )
            .outerjoin(
                SpeciesSiteObservationModel,
                SpeciesSiteModel.id_species_site
        ...
        for area in formatted_list.features:
            area["properties"]["creator_can_delete"] = (
                areas[formatted_list.features.index(area)][1] == 0
            )
...
def get_user_species_sites():
    ...
    try:
        ...
        formatted_list = prepare_list(
            list(map(lambda species_site: species_site[0], species_sites))
        )

        for species_site in formatted_list.features:
            observations_count = species_sites[
                formatted_list.features.index(species_site)
            ][1]
            species_site["properties"]["creator_can_delete"] = (
...
def get_user_observations():
    ...
    try:
        user_id = get_id_role_if_exists()
        observations = (
            SpeciesSiteObservationModel.query.join(
                SpeciesSiteModel,
                SpeciesSiteObservationModel.id_species_site
                == SpeciesSiteModel.id_species_site,
            )
...
def get_admin_species_sites():
    ...
    try:
        ...
        if request.args.get("area"):
            species_sites_query = species_sites_query.filter(
                SpeciesSiteModel.id_area == request.args.get("area")
            )

        species_sites = (
            species_sites_query.order_by(SpeciesSiteModel.id_species_site)
            .group_by(SpeciesSiteModel.id_species_site)
            .all()
        )
        ...
        for species_site in formatted_list.features:
            ...
            photos = (
                ...
                .join(
                    MediaModel,
                    MediaOnSpeciesSiteModel.id_media == MediaModel.id_media,
                )
                .all()
            )
            species_site["properties"]["photos"] = [
...
def get_admin_observations():
    ...
    try:
        ...
        observations_query = SpeciesSiteObservationModel.query.join(
            SpeciesSiteModel,
            SpeciesSiteModel.id_species_site
            == SpeciesSiteObservationModel.id_species_site,
        )
        ...
        if id_program > 0:
            observations_query = observations_query.join(
                AreaModel, AreaModel.id_area == SpeciesSiteModel.id_area
            ).filter(AreaModel.id_program == id_program)

        observations_count = observations_query.count()

        observations_query = observations_query.order_by(
            SpeciesSiteObservationModel.timestamp_create.desc()
        )

        if page_size:
            observations_query = observations_query.limit(page_size * 1)
        if page:
            observations_query = observations_query.offset(
                page * 1 * page_size * 1
            )
        ...
        for observation in observations:
            ...
            photos = (
                MediaModel.query.join(
                    MediaOnSpeciesSiteObservationModel,
                    MediaOnSpeciesSiteObservationModel.id_media
                    == MediaModel.id_media,
                )
                .filter(
                    MediaOnSpeciesSiteObservationModel.id_data_source
                    == observation.id_species_site_observation
                )
                .all()
            )
...
def get_areas_by_program(id):
    ...
    try:
        ...
        if program.is_private:
            user_id = get_id_role_if_exists()
            if user_id:
                areas_query = areas_query.outerjoin(
                    AreasAccessModel,
        ...
        areas = (
            areas_query.order_by(func.lower(AreaModel.name))
            .group_by(AreaModel.id_area)
            .all()
        )
...
def get_areas_observers(id):
    ...
    creator = UserModel.query.filter_by(id_user=area.id_role).first()

    response["creator"] = format_anon_user(creator, [])

    linked_users = UserModel.query.join(
        AreasAccessModel, AreasAccessModel.id_user == UserModel.id_user
    ).filter(
        AreasAccessModel.id_area == area.id_area,
...
def get_species_sites_by_program(id):
    ...
    try:
        ...
        if program.is_private:
            ...
            if logged_user_id:
                species_sites_query = species_sites_query.outerjoin(
                    AreasAccessModel,
                    AreasAccessModel.id_area == AreaModel.id_area,
                ).filter(
                    or_(
        ...
        species_sites = (
            species_sites_query.filter(
                or_(
                    SpeciesSiteModel.json_data.comparator.has_key("is_dead")
                    != True,
        ...
        for species_site in formatted_list.features:
            ...
            stages = (
                ...
                .outerjoin(
                    StagesStepModel,
                    and_(
                        StagesStepModel.id_species_stage
                        == SpeciesStageModel.id_species_stage,
                        StagesStepModel.order > 1,
                    ),
                )
                .join(
                    SpeciesSiteModel,
                    SpeciesSiteModel.cd_nom == SpeciesStageModel.cd_nom,
                )
                .outerjoin(
                    SpeciesSiteObservationModel,
                    and_(
                        SpeciesSiteObservationModel.id_species_site
                        == SpeciesSiteModel.id_species_site,
                        StagesStepModel.id_stages_step
                        == SpeciesSiteObservationModel.id_stages_step,
                        SpeciesSiteObservationModel.date.between(
...
def get_observations_by_program(id):
    ...
    try:
        ...
        if program.is_private:
            user_id = get_id_role_if_exists()
            if user_id:
                observations_query = observations_query.outerjoin(
                    AreasAccessModel,
                    AreasAccessModel.id_area == AreaModel.id_area,
                ).filter(
                    or_(
        ...
        observations_query = observations_query.group_by(
            SpeciesSiteObservationModel.id_species_site_observation
        ).order_by(SpeciesSiteObservationModel.timestamp_create.desc())
...
def get_area(pk):
    ...
    try:
        ...
        for species_site in species_sites["features"]:
            last_observation = (
                SpeciesSiteObservationModel.query.filter_by(
                    id_species_site=species_site["properties"][
                        "id_species_site"
                    ]
                )
                .order_by(SpeciesSiteObservationModel.timestamp_create.desc())
            ...
            if last_observation is not None:
                species_site["properties"][
                    "last_observation"
                ] = last_observation.as_dict(True)
...
def post_area():
    ...
    try:
        ...
        try:
            coordinates = request_data.get("geometry", {}).get(
                "coordinates", []
            )
            ...
            try:
                new_area.municipality = get_municipality_id_from_wkb(
                    from_shape(p, srid=4326)
                )
            except Exception as e:
...
def update_area():
    try:
        ...
        if (
            current_user.email
            != UserModel.query.get(area.first().id_role).email
            and len(area_user_access) == 0
        ...
        try:
            coordinates = update_data.get("geometry", {}).get(
                "coordinates", []
            )
            ...
            try:
                update_area["municipality"] = get_municipality_id_from_wkb(
                    from_shape(p, srid=4326)
                )
            except Exception as e:
...
def delete_area(area_id):
    ...
    try:
        ...
        if area and (
            (
                current_user.email == area.UserModel.email
                and linked_observations_number == 0
            )
...
def get_species_site(pk):
    ...
    try:
        ...
        if (
            request.args.get("with_observations") is None
            or request.args.get("with_observations") != "false"
        ):
        ...
        if request.args.get("with_stages"):
            ...
            for stage in stages.features:
                steps = prepare_list(
                    StagesStepModel.query.filter_by(
                        id_species_stage=stage["properties"][
                            "id_species_stage"
                        ]
                    )
                ...
                for step in steps.features:
                    photos = (
                        MediaOnStagesStepsModel.query.filter_by(
                            id_data_source=step["properties"]["id_stages_step"]
                        )
...
def post_species_site():
    ...
    try:
        ...
        if id_role is not None:
            ...
            if (
                new_species_site.obs_txt is None
                or len(new_species_site.obs_txt) == 0
            ):
        ...
        try:
            ...
            if request_data.get("photos[0]", None) is not None:
                ...
                for index in range(max_length):
                    if (
                        request_data.get("photos[" + str(index) + "]", None)
                        is None
                    ):
                        break
                    files.append(
                        request_data.get("photos[" + str(index) + "]")
                    )
        ...
        return {
            "message": "New species site created.",
...
def update_species_site():
    try:
        ...
        species_site = SpeciesSiteModel.query.filter_by(
            id_species_site=update_data.get("id_species_site", 0)
        )
        area_user_access = (
            ...
            .outerjoin(
                AreaModel,
                or_(
                    AreaModel.id_area == AreasAccessModel.id_area,
            ...
            .join(
                SpeciesSiteModel,
                or_(
                    SpeciesSiteModel.id_area == AreaModel.id_area,
        ...
        if (
            current_user.email
            != UserModel.query.get(species_site.first().id_role).email
            and len(area_user_access) == 0
            and current_user.admin != 1
        ):
            return ("unauthorized"), 403
        ...
        try:
            id_media_to_delete = json.loads(
                update_data.get("delete_media", "[]")
            )
            if len(id_media_to_delete):
                db.session.query(MediaOnSpeciesSiteModel).filter(
                    MediaOnSpeciesSiteModel.id_media.in_(
            ...
            current_app.logger.warning(
                "[update_species_site] delete media ", e
            )
            raise GeonatureApiError(e)
        ...
        try:
            ...
            if update_data.get("photos[0]", None) is not None:
                ...
                for index in range(max_length):
                    if (
                        update_data.get("photos[" + str(index) + "]", None)
                        is None
                    ):
                        break
            ...
            current_app.logger.debug(
                "[update_species_site] Species site UPLOAD FILE {}".format(
                    file
                )
            )
        except Exception as e:
            current_app.logger.warning(
                "[update_species_site] Species site ERROR ON FILE SAVING",
                str(e),
            )
...
def delete_species_site(species_site_id):
    ...
    try:
        species_site = (
            ...
            .join(
                UserModel,
                SpeciesSiteModel.id_role == UserModel.id_user,
                full=True,
            )
            .first()
        )
        ...
        linked_observations_number = (
            SpeciesSiteObservationModel.query.filter_by(
                id_species_site=species_site_id
            ).count()
        )

        if species_site and (
            (
                current_user.email == species_site.UserModel.email
                and linked_observations_number == 0
            )
            or current_user.admin
        ):
            SpeciesSiteModel.query.filter_by(
                id_species_site=species_site_id
            ).delete()
            db.session.commit()
...
@areas_api.route(
    "/species_sites/<int:species_site_id>/observations", methods=["POST"]
)
@json_resp
@jwt_required(optional=True)
def post_observation(species_site_id):
    try:
        ...
        if id_role is not None:
            ...
            if (
                new_observation.obs_txt is None
                or len(new_observation.obs_txt) == 0
            ):
        ...
        result = SpeciesSiteObservationModel.query.get(
            new_observation.id_species_site_observation
        )
...
def update_observation():
    try:
        ...
        observation = SpeciesSiteObservationModel.query.filter_by(
            id_species_site_observation=update_data.get(
                "id_species_site_observation", 0
            )
        )
        if (
            current_user.email
            != UserModel.query.get(observation.first().id_role).email
            and current_user.admin != 1
        ):
            return ("unauthorized"), 403
        ...
        update_observation["id_stages_step"] = update_data.get(
            "stages_step_id", None
        )
        ...
        try:
            ...
            if len(id_media_to_delete):
                db.session.query(MediaOnSpeciesSiteObservationModel).filter(
                    MediaOnSpeciesSiteObservationModel.id_media.in_(
                        tuple(id_media_to_delete)
                    ),
                    MediaOnSpeciesSiteObservationModel.id_data_source
...
def delete_observation(observation_id):
    ...
    try:
        ...
        if observation and (
            current_user.email == observation.UserModel.email
            or current_user.admin
        ):
            SpeciesSiteObservationModel.query.filter_by(
                id_species_site_observation=observation_id
            ).delete()
            db.session.commit()
...
def export_areas_xls(user_id):
    ...
    try:
        ...
        if filter_by_user:
            areas_query = areas_query.outerjoin(
                AreasAccessModel, AreasAccessModel.id_area == AreaModel.id_area
            ).filter(
                or_(
                    AreaModel.id_role == user_id,
                    AreasAccessModel.id_user == user_id,
        ...
        fields = (
            ...
            {
                "col_name": "Programme",
                "getter": lambda area: area.program.title,
            },
            {
                "col_name": "Créateur",
                "getter": lambda area: str(area.obs_txt)
                + " ("
                + str(area.id_role)
                + ")",
            },
            ...
            {
                "col_name": "Coord. x",
                "getter": lambda area: str(area.coordinates[0]),
            },
            {
                "col_name": "Coord. y",
                "getter": lambda area: str(area.coordinates[1]),
            },
            {
                "col_name": "Ville",
                "getter": lambda area: area.municipality_data.area_name
        ...
        json_keys = list(
            set([key for area in areas for key in area.json_data.keys()])
        )
        row, col = 0, 0
        for field in fields:
        ...
        if filter_by_user:
            species_sites_query = (
                species_sites_query.join(
                    AreaModel, SpeciesSiteModel.id_area == AreaModel.id_area
                )
                .outerjoin(
                    AreasAccessModel,
                    AreasAccessModel.id_area == AreaModel.id_area,
        ...
        species_sites = species_sites_query.order_by(
            func.lower(SpeciesSiteModel.name)
        ).all()

        basic_fields = (
            {"col_name": "ID", "getter": lambda s: s.id_species_site},
            {
                "col_name": "Observateur",
                "getter": lambda s: str(s.obs_txt)
                + " ("
                + str(s.id_role)
                + ")",
            },
            ...
            {
                "col_name": "Zone",
                "getter": lambda s: s.area.name if s.area else "",
            },
            {
                "col_name": "Espèce",
                "getter": lambda s: (
                    str(s.species.nom_vern)
                    + " ("
                    + str(s.species.nom_complet)
                    + ")"
                )
                if s.species
                else "",
        ...
        json_keys = list(
            set(
                [
                    ...
                    for key in species_site.json_data.keys()
                ]
            )
        )
        row, col = 0, 0
        ...
        basic_fields = (
            {
                "col_name": "ID",
                "getter": lambda s: s.id_species_site_observation,
            },
            {
                "col_name": "Date obs",
            ...
            },
            {
                "col_name": "Zone",
                "getter": lambda s: (
                    s.species_site.area.name if s.species_site.area else ""
                )
                if s.species_site
                else "",
            },
            {
                "col_name": "Individu",
                "getter": lambda s: s.species_site.name
                if s.species_site
                else "",
            },
            {
                ...
                "getter": lambda s: (
                    (
                        str(s.species_site.species.nom_vern)
                        + " ("
                        + str(s.species_site.species.nom_complet)
                        + ")"
                    )
            ...
            {
                "col_name": "Stade",
                "getter": lambda s: s.stages_step.species_stage.name
                if s.stages_step and s.stages_step.species_stage
                else "",
            },
            {
                "col_name": "Etape",
                "getter": lambda s: s.stages_step.name
                if s.stages_step
                else "",
            },
        )
        json_keys = list(
            set(
                [
                    key
                    for observation in observations
                    for key in observation.json_data.keys()
                ]
            )
        )
        row, col = 0, 0
        ...
        if not filter_by_user:
            ...
            observers = observers_query.order_by(
                UserModel.timestamp_create.desc()
            ).all()
            ...
            if current_user.admin:
                basic_fields.append(
                    {
                        "col_name": "Zones créées",
                        "getter": lambda s: s.created_areas,
                    }
                )
                basic_fields.append(
                    {
                        "col_name": "Zones associées",
                        "getter": lambda s: s.areas_access,
                    }
                )
                basic_fields.append(
                    {
                        "col_name": "Statut",
                        "getter": lambda s: "Admin"
                        if s.admin
                        else "Utilisateur",
                    }
                )
            ...
            for observer in observers:
                ...
                created_areas = AreaModel.query.filter(
                    AreaModel.id_role == observer.id_user
                ).all()
                for area in created_areas:
                    observer.created_areas += str(area.id_area) + " - "

                observer.areas_access = ""
                areas_access = AreasAccessModel.query.filter(
                    AreasAccessModel.id_user == observer.id_user
                ).all()
