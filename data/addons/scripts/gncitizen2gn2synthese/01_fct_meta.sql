...
VALUES
(NULL, 'gnc_default_acquisition_framework', 'Cadre d''acquisition par défaut pour les données GeoNature-citizen'
...
CREATE
    OR REPLACE FUNCTION gnc_core.fct_get_or_insert_basic_acquisition_framework(_name      TEXT,
...
BEGIN
    IF
        ...
        RAISE
            NOTICE 'Acquisition framework named % already exists', _name;
    ELSE
        ...
        RAISE
            NOTICE 'Acquisition framework named % inserted with id %', _name, the_new_id;
    END IF;
    RETURN the_new_id;
END
$$
...
COMMENT
    ON FUNCTION gnc_core.fct_get_or_insert_basic_acquisition_framework(_name TEXT, _desc TEXT, _startdate DATE) IS 'function to basically create acquisition framework'
;
...
CREATE
    OR REPLACE FUNCTION gnc_core.fct_get_or_insert_dataset_from_title(_title TEXT, _acquisition_framework TEXT DEFAULT NULL) RETURNS INTEGER
AS
$$
DECLARE
    the_id_dataset INT ;
    the_id_acquisition_framework
                   INT;
BEGIN
    ...
    */
    ...
    RAISE
        NOTICE '<fct_c_get_or_insert_dataset_from_title> Data dataset is % ', _title;

    IF
        (
            SELECT EXISTS(SELECT 1 FROM gn_meta.t_datasets WHERE dataset_shortname LIKE _title)) THEN
/* Si le JDD par défaut existe déjà, on récupère son ID */
        ...
        FROM
            gn_meta.t_datasets
        WHERE
            dataset_shortname LIKE _title;
        RAISE
            NOTICE '<fct_c_get_or_insert_dataset_from_title> Dataset with shortname % exists with get ID : %', _title, the_id_dataset;
    ELSE
        INSERT INTO
            gn_meta.t_datasets(id_acquisition_framework
                              ...
                              , meta_create_date)
        VALUES
        (gnc_core.fct_get_or_insert_basic_acquisition_framework(
        ...
        RAISE
            NOTICE '<fct_c_get_or_insert_dataset_from_title> Data dataset doesn''t exists, new dataset with shortname % created with ID : %', _title, the_id_dataset;
    END IF;
...
COMMENT
    ON FUNCTION gnc_core.fct_get_or_insert_dataset_from_title(_title TEXT,_default_acquisition_framework TEXT) IS 'function to basically create acquisition framework'
;
...
CREATE
    OR REPLACE FUNCTION gnc_core.fct_get_id_acquisition_framework_by_name(_name TEXT) RETURNS INTEGER
    LANGUAGE plpgsql
AS
$$
DECLARE
...
COMMENT
    ON FUNCTION gnc_core.fct_get_id_acquisition_framework_by_name(_name TEXT) IS 'function to get acquisition framework id by name'
;
...
CREATE
    OR REPLACE FUNCTION gnc_core.fct_get_id_dataset_by_shortname(_title TEXT) RETURNS INTEGER
    LANGUAGE plpgsql
AS
$$
DECLARE
...
BEGIN
    ...
    FROM
        gn_meta.t_datasets
    WHERE
        dataset_shortname ILIKE _title;
    RETURN theiddataset;
END;
$$
;
...
COMMENT
    ON FUNCTION gnc_core.fct_get_id_dataset_by_shortname(_title TEXT) IS 'function to get dataset id by shortname'
;
...
CREATE
    OR REPLACE FUNCTION gnc_core.fct_tri_upsert_meta_datasets() RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
DECLARE
...
BEGIN
    ...
    ELSE
        ...
        SET
            dataset_shortname = new.title
        WHERE
            dataset_shortname = old.title;
    END IF;
    RETURN new;
END;
$$
...
REPLACE
TRIGGER tri_upsert_meta_datasets
    AFTER
